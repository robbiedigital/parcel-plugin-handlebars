...
function loadUserConfig() {
  ...
  if (fs.existsSync('./hbs.config.js')) { // eslint-disable-line no-sync
    return JSON.parse(fs.readFileSync('./hbs.config.js')); // eslint-disable-line no-sync
  }

  if (fs.existsSync('./hbs.config.json')) { // eslint-disable-line no-sync
    return JSON.parse(fs.readFileSync('./hbs.config.json')); // eslint-disable-line no-sync
  }
